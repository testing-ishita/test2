...
* Filename:	 lz.c
*
* Description:	LZ77 Compress/Decompress functions from the free
*               Basic Compression Library by Marcus Geelnard, see below.
*               
*				CCT Modifications:
*               1. Function return value changed to unsigned int.
*               2. INCLUDE_COMPRESS_FUNCS conditional added.
*               3. Added LZ_UncompressImage() function.
*
* $Revision: 1.1 $
*
* $Date: 2013-09-04 07:43:05 $
*
* $Source: /home/cvs/cvsroot/CCT_BIT_2/lzlib/lz.c,v $
*
...
/* Note: these embedded symbol names are produced when objcopy creates */
/* compress.o, and will change if a different input filename is used   */
extern unsigned char binary___bin_compress_lz_start[];
extern unsigned char binary___bin_compress_lz_end[];

#undef INCLUDE_COMPRESS_FUNCS
...
static unsigned int _LZ_WriteVarSize( unsigned int x, unsigned char * buf )
{
    unsigned int y;
    int num_bytes, i, b;
...
static unsigned int _LZ_ReadVarSize( unsigned int * x, unsigned char * buf )
{
...
* LZ_UncompressImage
*
* Uncompresses the image file in the external binary array to the given start
* address in memory.
*
*
* RETURNS: None
* 
*******************************************************************************/
void LZ_UncompressImage( unsigned int loadAddress )
{
	unsigned char *p;
	unsigned int length;
	

	/* No range checking, we assume the given loadAddress is a valid address */

	/* Set data start position and length. */
	/* There is a 12 byte header at the begining which we adjust for.    */
	/* Note: the embedded symbol names are produced when objcopy creates */
	/* compress.o, and will change if a different input filename is used */
	
	p = (unsigned char *) &binary___bin_compress_lz_start[12];
	length = (binary___bin_compress_lz_end - binary___bin_compress_lz_start) - 12;

	/* Uncompress the image */
	LZ_Uncompress( p, (unsigned char *) loadAddress, length );
