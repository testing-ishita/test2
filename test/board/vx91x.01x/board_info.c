...
/*********************************************************************
 * brdGetSensorThresholds: Gets Threshold Values for Sensor x
 * ptr	            	 : pointer to Sensor_Threshold structure
 * RETURNS          	 : E__OK or E__FAIL
 */

UINT32 brdGetSensorThresholds (void* ptr)
{
	UINT32	rt;
	UINT8   buffer[TOTAL_THRESHOLDS_SIZE]; // this board only has two temperature sensors so there
										   // are four 4 byte values
	Sensor_Threshold *tPtr;

	tPtr = (Sensor_Threshold*)ptr;
    rt   = getExtendedField( EFIELD_TEMPERATURE_GRADE_DATA, buffer, TOTAL_THRESHOLDS_SIZE );

	if(rt == E__OK)
	{
		if((tPtr->sensor * THRESHOLDS_SIZE) < TOTAL_THRESHOLDS_SIZE)
		{
			tPtr->UCr = *((UINT32*)(buffer + (tPtr->sensor * 8)));
			tPtr->LCr = *((UINT32*)(buffer + (tPtr->sensor * 8)+ 4));
		}
		else
			rt = SeTh_WRONG_SENSOR_NUMBER; // wrong sensor number
	}

	return rt;

} /* brdGetSensorThresholds () */
...
{
  if(((BIOSInfo.BIOS_ver >> 16)& 0x000000ff) == 0x58)
      sprintf((char*)ptr, "%X.%02X.%02Xrc",
                     ((BIOSInfo.BIOS_ver >> 8)& 0x000000ff),
                     (BIOSInfo.BIOS_ver & 0x000000ff),
                     ( (BIOSInfo.BIOS_ver >> 24) & 0x000000ff)
                     );
/* 'V' release version */
  if(((BIOSInfo.BIOS_ver >> 16)& 0x000000ff) == 0x56)
    sprintf((char*)ptr, "%X.%02X.%02X",
		   ((BIOSInfo.BIOS_ver >> 8)& 0x000000ff),
		   (BIOSInfo.BIOS_ver & 0x000000ff),
		   ( (BIOSInfo.BIOS_ver >> 24) & 0x000000ff)
		   );
