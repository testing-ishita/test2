...
#if 0
	/* Serial console can be COM1 or COM2 */
...
UINT32 brdSkipEthInterface (void *ptr)
{
		...
		(bGetTestHandler() == TH__MTH ))
	{
		...
		if( (((SkipEthIf_PARAMS*)ptr)->dDidVid   == DIDVID_82580_EB) &&
		   (((SkipEthIf_PARAMS*)ptr)->bInstance == 1) && (bDev == 0) && (bFunc == 0)  )
		{
			return E__OK;
		}
		else if ( dTemp == TSI721_VID_DID)
		{
			...
			sprintf ( achBuffer, "bBus:%d,bDev:%d,bFunc:%d,dDidVid:%x\n",bBus,bDev,bFunc, ((SkipEthIf_PARAMS*)ptr)->dDidVid );
			...
			{
#ifdef DEBUG
				sprintf ( achBuffer, "1. SRIO DIDVID:%x\n",dTemp);
				vConsoleWrite (achBuffer);
#endif
				return E__OK;
			}
		...
		{
#ifdef DEBUG
			sprintf ( achBuffer, "2. SRIO DIDVID:%x\n",dTemp);
			vConsoleWrite (achBuffer);
#endif
			  return (1);
		}
...
UINT32 brdMemfromGPIO (void *ptr)
{
	...
	if (dPciReadReg(pfa, 0x00, 0x03) == 0x01548086)
	{
