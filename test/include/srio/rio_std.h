...
struct rio_switch {
	...
	int (*get_entry) (struct rio_mport *mport, UINT16 destid, UINT8 hopcount,
			  UINT16 table, UINT16 route_destid, UINT8 *route_port);
	int (*clr_table) (struct rio_mport *mport, UINT16 destid, UINT8 hopcount,
			  UINT16 table);
...
struct rio_dev {
	...
	struct dev_name dev;	/* LDM device structure */
...
struct rio_dbell {
	struct list_head node;
	struct resource *res;
	void (*dinb) (struct rio_mport *mport, void *dev_id, UINT16 src, UINT16 dst, UINT16 info);
	void *dev_id;
};
 ...
 * @ops: configuration s	return 0;pace functions
...
struct rio_mport {
	...
	struct dma_async_tx_descriptor* (*prep_rio_sg)(struct dma_chan *dchan,
 ...
 */
struct rio_ops {
	int (*lcread) (struct rio_mport *mport, int index, UINT32 offset, int len,
			UINT32 *data);
	int (*lcwrite) (struct rio_mport *mport, int index, UINT32 offset, int len,
			UINT32 data);
	int (*cread) (struct rio_mport *mport, int index, UINT16 destid,
			UINT8 hopcount, UINT32 offset, int len, UINT32 *data);
	int (*cwrite) (struct rio_mport *mport, int index, UINT16 destid,
			UINT8 hopcount, UINT32 offset, int len, UINT32 data);
	//int (*pwenable) (struct rio_mport *mport, int enable);
	int (*map_inb)(struct rio_mport *mport, dma_addr_t lstart,
			UINT64 rstart, UINT32 size, UINT32 flags);
	void (*unmap_inb)(struct rio_mport *mport, dma_addr_t lstart);
	int (*get_outb_pref_asinfo)( struct rio_mport *mport,UINT64 *addr, UINT64 *size );
	int (*get_outb_npref_asinfo)( struct rio_mport *mport,UINT64 *addr, UINT64 *size );
	int (*map_outb)( struct rio_mport *mport, UINT8 win, UINT16 destid, UINT64 physaddr,
			UINT64 rstart, UINT64 size, UINT32 flags );
	void (*unmap_outb)(struct rio_mport *mport, UINT8 win );
	int (*config_outb_zone)( struct rio_mport *mport, UINT8 win, UINT8 zone, UINT16 destid,
...
struct rio_driver {
	...
	int (*suspend) (struct rio_dev * dev, UINT32 state);
	int (*resume) (struct rio_dev * dev);
	int (*enable_wake) (struct rio_dev * dev, UINT32 state, int enable);
...
union rio_pw_msg {
	struct {
		UINT32 comptag;	/* Component Tag CSR */
		UINT32 errdetect;	/* Port N Error Detect CSR */
		UINT32 is_port;	/* Implementation specific + PortID */
		UINT32 ltlerrdet;	/* LTL Error Detect CSR */
		UINT32 padding[12];
	} em;
	UINT32 raw[RIO_PW_MSG_SIZE/sizeof(UINT32)];
};
...
struct rio_dma_ext {
	UINT16 destid;
	UINT64 rio_addr;	/* low 64-bits of 66-bit RapidIO address */
	UINT8  rio_addr_u;  /* upper 2-bits of 66-bit RapidIO address */
...
extern void rio_unregister_mport(struct rio_mport *port);
