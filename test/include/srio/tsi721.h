...
//#define TSI721_BUF_DESC_BASE   0x00E00000
//#define TSI721_DMA_STATUS_BASE 0x00F00000


#define TSI721_BUF_DESC_BASE     0x05000000
#define TSI721_DMA_STATUS_BASE 0x05100000
...
#define IDB_QSIZE		1024 /* Inbound Doorbell Queue size */
#define TSI721_ZONE_TIMEOUT	0x80000000
...
#define DBELL_SID(buf)		(((UINT8)buf[2] << 8) | (UINT8)buf[3])
#define DBELL_TID(buf)		(((UINT8)buf[4] << 8) | (UINT8)buf[5])
#define DBELL_INF(buf)		(((UINT8)buf[0] << 8) | (UINT8)buf[1])
...
#define TSI721_RDTYPE_NREAD     1
#define TSI721_RDTYPE_MTREAD	2

#define TSI721_WRTYPE_NWRITE    1
#define TSI721_WRTYPE_MTWRITE	2
#define TSI721_WRTYPE_NWRITE_R	4
...
#define TSI721_DMAD_RADDR0	0xc0000000
	...
	union {
		UINT32 raddr_lo;	   /* if DTYPE == (1 || 2) */
		UINT32 next_lo;		   /* if DTYPE == 3 */
	};
...
#define TSI721_DMAD_HOPCNT	0xff000000
	...
	union {
		UINT32 raddr_hi;	   /* if DTYPE == (1 || 2) */
		UINT32 next_hi;		   /* if DTYPE == 3 */
	};
	...
	union {
		...
		UINT32 data[4];		   /* if DTYPE == 2 */
		UINT32    reserved[4];	   /* if DTYPE == 3 */
	};
} __attribute__ ((aligned (32))); /*__aligned(32);*/
...
} __attribute__ ((aligned (64))); /*__aligned(64);*/
...
#define TSI721_OMD_TT		0x0c000000
	...
	union {
		UINT32 bufptr_lo;	/* if DTYPE == 4 */
		UINT32 next_lo;		/* if DTYPE == 5 */
	};

	union {
		UINT32 bufptr_hi;	/* if DTYPE == 4 */
		UINT32 next_hi;		/* if DTYPE == 5 */
	};
...
#define TSI721_BDMA_BD_RING_SZ	(4096*4)
...
struct tsi721_omsg_ring {
	...
	void		*sts_base;
	dma_addr_t	sts_phys;
	UINT32		sts_size; /* # of allocated status entries */
	UINT32		sts_rdptr;
...
struct tsi721_device {
	...
	UINT32		flags;
	volatile void  *regs;
	volatile unsigned long pfmem_base;
	UINT64	 	pf_size;
	volatile unsigned long	npfmem_base;
	UINT64		npf_size;
#ifdef CONFIG_PCI_MSI
	struct msix_irq	msix[TSI721_VECT_MAX];
#endif
	/* Doorbells */
	volatile void /*__iomem*/ *odb_base;
	volatile void		*idb_base;
	dma_addr_t	idb_dma;
	/*struct tasklet_struct idb_work;*/
	UINT32		db_discard_count;

	/* Inbound Port-Write */
	/*struct work_struct pw_work;*/
	/*struct kfifo	pw_fifo;*/
	/*spinlock_t	pw_fifo_lock;*/
	UINT32		pw_discard_count;
