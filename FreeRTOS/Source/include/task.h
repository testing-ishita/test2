...
* Filename: 	tasks.h
* 
* Description:	FreeRTOS task header, adapted for use with CCT boards.
*
* $Revision: 1.2 $
*
* $Date: 2013-10-08 07:08:26 $
*
* $Source: /home/cvs/cvsroot/CCT_BIT_2/FreeRTOS/Source/include/task.h,v $
*
 ...
 * \ingroup SchedulerControl
 */
#define taskENTER_CRITICAL	vTaskLockTake
 ...
 * \ingroup SchedulerControl
 */
#define taskEXIT_CRITICAL	vTaskLockGive
 ...
 * \ingroup SchedulerControl
 */
#define taskDISABLE_INTERRUPTS( lkey )	portDISABLE_INTERRUPTS( lkey )
 ...
 * \ingroup SchedulerControl
 */
#define taskENABLE_INTERRUPTS( lkey )	portENABLE_INTERRUPTS( lkey )
 ...
 * \ingroup Tasks
 */
#define xTaskCreate( cpuNo, pvTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask ) xTaskGenericCreate( (cpuNo), ( pvTaskCode ), ((signed char *) pcName ), ( usStackDepth ), ( pvParameters ), ( uxPriority ), ( pxCreatedTask ), ( NULL ), ( NULL ) )
 ...
 * \ingroup SchedulerControl
 */
void vTaskSuspendCpu( portSHORT cpuNo ) PRIVILEGED_FUNCTION;
 ...
 * \ingroup SchedulerControl
 */
signed portBASE_TYPE xTaskResumeCpu( portSHORT cpuNo ) PRIVILEGED_FUNCTION;
 ...
 * a ready list.
 */
void vTaskIncrementTick( portSHORT cpuNo ) PRIVILEGED_FUNCTION;
 ...
 * period.
 */
void vTaskPlaceOnEventList( portSHORT cpuNo, const xList * const pxEventList, portTickType xTicksToWait ) PRIVILEGED_FUNCTION;
 ...
 */
signed portBASE_TYPE xTaskRemoveFromEventList( portSHORT *psCpuNo, const xList * const pxEventList, int fromIsr ) PRIVILEGED_FUNCTION;
 ...
 */
void vTaskSwitchContext( portSHORT cpuNo ) PRIVILEGED_FUNCTION;
 ...
 */
void vTaskSetTimeOutState( portSHORT cpuNo, xTimeOutType * const pxTimeOut ) PRIVILEGED_FUNCTION;

/*
 * Compare the time status now with that previously captured to see if the
 * timeout has expired.
 */
portBASE_TYPE xTaskCheckForTimeOut( portSHORT cpuNo, xTimeOutType * const pxTimeOut, portTickType * const pxTicksToWait ) PRIVILEGED_FUNCTION;

/*
 * Shortcut used by the queue implementation to prevent unnecessary call to
 * taskYIELD();
 */
void vTaskMissedYield( portSHORT cpuNo ) PRIVILEGED_FUNCTION;
 ...
 */
void vTaskPriorityInherit( xTaskHandle * const pxMutexHolder, int fromIsr ) PRIVILEGED_FUNCTION;

/*
 * Set the priority of a task back to its proper priority in the case that it
 * inherited a higher priority while it was holding a semaphore.
 */
void vTaskPriorityDisinherit( xTaskHandle * const pxMutexHolder, int fromIsr ) PRIVILEGED_FUNCTION;

/*
 * Generic version of the task creation function which is in turn called by the
 * xTaskCreate() and xTaskCreateRestricted() macros.
 */
signed portBASE_TYPE xTaskGenericCreate( portSHORT cpuNo, pdTASK_CODE pvTaskCode, const signed char * const pcName, unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority, xTaskHandle *pxCreatedTask, portSTACK_TYPE *puxStackBuffer, const xMemoryRegion * const xRegions ) PRIVILEGED_FUNCTION;

/*
 *	Sets scheduler running
 */ 
void vTaskStartSchedulerRunning( portSHORT cpuNo ) PRIVILEGED_FUNCTION;

/*
 *	Turns context switching on/off.
 */ 
void vTaskSetContextSwitching( portSHORT cpuNo, int value ) PRIVILEGED_FUNCTION;

/*
 *	Get/set first execute status
 */ 
short vTaskGetFirstExecute( volatile void *pTCB ) PRIVILEGED_FUNCTION;
void vTaskSetFirstExecute( volatile void *pxTCB, short value ) PRIVILEGED_FUNCTION;

/*
 *	Takes/gives the task control spinlock for the given CPU.
 *	Used instead of the taskENTER_CRITICAL and taskENTER_CRITICAL macros 
 */ 
void vTaskLockTake( portSHORT cpuNo, int fromIsr ) PRIVILEGED_FUNCTION;
void vTaskLockGive( portSHORT cpuNo, int fromIsr ) PRIVILEGED_FUNCTION;

/*
 *	Get task number
 */ 
unsigned portBASE_TYPE uxTaskNumGet( portSHORT cpuNo, xTaskHandle pxTask ) PRIVILEGED_FUNCTION;

/*
 * Initializes the task data.
 * 
 */
void vTaskDataInit( UINT8 bCpusAvailable ) PRIVILEGED_FUNCTION;
