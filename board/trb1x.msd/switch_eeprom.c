...
/* Version control:
 *
 * $Header: /home/cvs/cvsroot/CCT_BIT_2/board/trb1x.msd/switch_eeprom.c,v 1.1 2013-11-25 14:21:34 mgostling Exp $
 *
...
TEST_INTERFACE (PcieSwitchEepromTest, "PCIe Switch EEPROM Access Test")
{
	...
	dStatus	= E__OK;
	dStatus = iPciFindDeviceById (1, 0x10B5, 0x8717, &pfa);
	if (dStatus == E__OK)
	{
...
#endif
				dStatus = PCIE_SWITCH_NO_EEPROM_ERROR;
				break;
...
#endif
				dStatus = PCIE_SWITCH_EEPROM_RSVD_ERROR;
				break;
	...
	if (dStatus == E__OK)
	{
		...
		if (((dRegVal & 0x00040000L) != 0x00000000L ) || bTimeout == 0)
			dStatus = PCIE_SWITCH_EEPROM_TIMEOUT_ERROR;
		else
		{
