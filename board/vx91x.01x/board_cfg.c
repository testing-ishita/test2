...
/* Version control:
 *
 * $Header: /home/cvs/cvsroot/CCT_BIT_2/board/vx91x.01x/board_cfg.c,v 1.1 2014-05-15 09:42:03 jthiru Exp $
 *
 * $Log: board_cfg.c,v $
...
static ENXMC_GPIO_LOOPBACK_INFO xmcSite1LoopbackInfo = 
{
	...
	0x02222000L,								// PLX PEX819 GPIO_CONFIG2 register: GPIO 16,18,20,22,24,26,28 outputs, 17,19,21,23,25,27,29 inputs
...
static ENXMC_GPIO_LOOPBACK_INFO xmcSite2LoopbackInfo = 
{
	...
	0x220A0A0AL,								// PLX PEX819 GPIO_CONFIG1 register: GPIO 0,1,4,5,8,9,12,14 outputs, 2,3,6,7,10,11,13,15 inputs
	0x02222222L,								// PLX PEX819 GPIO_CONFIG2 register: GPIO 16,18,20,22,24,26,28 outputs, 17,19,21,23,25,27,29 inputs
	0x15555333L,								// bitmap of GPIOs configured as outputs
	0x2AAAACCCL,								// bitmap of GPIOs configured as inputs
};
 ...
 * */
	...
	while (asSccParams[i].bPort != 0)
	{
		...
		//skip debug port
		if((asSccParams[i].bPort - 1) == bDebugPort)
...
UINT32 brdGetAhciPortInfo (void *ptr)
{
  *(UINT32*)ptr = 0x38; /* To be used for PI AHCI reg */
  return E__OK;
}
