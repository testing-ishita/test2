...
/* Version control:
 *
 * $Header: /home/cvs/cvsroot/CCT_BIT_2/board/vpb1x.msd/hw_init.c,v 1.3 2015-03-12 15:25:19 hchalla Exp $
 *
 * $Log: hw_init.c,v $
 * Revision 1.3  2015-03-12 15:25:19  hchalla
 * Commented out unwanted code.
 *
 * Revision 1.2  2015-02-27 17:18:44  hchalla
...
static void rtcInit(void)
{
	...
	temp = sysInPort8 (0x71);//A read will set the VRT bit of D register 
	...
	//if(temp > 59)
		//sysOutPort8 (0x71, 0x00);
	sysOutPort8 (0x71, 0x00); //added by RS
	...
	//temp = bcdToDecimalInit(sysInPort8(0x71));
	//if(temp > 31)
		//sysOutPort8 (0x71, 0x01);//this can be an issue when temp=0, as  thec basic test for day of month will fail as comparison is for 1<=day<=31
	sysOutPort8 (0x71, 0x01); //added by RS. This will probably fix the BUG when day register value is coming as zero and rtc basic test is failing

	//month
	sysOutPort8 (0x70, 0x08);
	//temp = bcdToDecimalInit(sysInPort8(0x71));
	//if(temp > 12)
		//sysOutPort8 (0x71, 0x01);//this can be an issue when temp=0, as  thec basic test for  month will fail as comparison is for 1<=month<=12
	sysOutPort8 (0x71, 0x01); //added by RS. This will fix the probable BUG when month register value is coming as zero and rtc basic test failing	
	...
	sysOutPort8 (0x71, 0x13); //added by RS. Setting the year to a known value
...
UINT32 brdHwInit (void *ptr)
{
	...
	dTemp = 0x01EA0001;										// turn off unused PCIe Bridge 2
...
#pragma GCC diagnostic pop
	...
	pfa = PCI_MAKE_PFA (0, 0, 0);
	prev11 = PCI_READ_WORD (pfa, 0xbc);
	PCI_WRITE_WORD (pfa, 0xbc, (UINT16)dPciTopOfMem);
	prev12 = PCI_READ_WORD (pfa, 0xbc);
#endif //#if 0
	return E__OK;
}
