...
/* Version control:
 *
 * $Header: /home/cvs/cvsroot/CCT_BIT_2/board/ppb1x.msd/board_info.c,v 1.2 2015-04-01 16:44:29 hchalla Exp $
 *
 * $Log: board_info.c,v $
 ...
 *
 * Revision 1.4  2013-11-25 14:11:33  mgostling
 * Added support for MAKE_BIT builds.
 *
...
#include "../ppb1x.msd/board_info.h"
...
#ifdef DEBUG	
	sysDebugPrintf("%s: my_slot: 0x%04x\n", __FUNCTION__, my_slot);
#endif
...
UINT32 brdGetSlotId( void* ptr )
{
	...
	if (bSysCon == 1)
	{
		...
		cpciIpmbAddrs = bGetAMCIpmbAddrs();

	#ifdef DEBUG
		sysDebugPrintf("%s: my_slot: 0x%04x cpciIpmbAddrs: 0x%02x\n", __FUNCTION__, my_slot, cpciIpmbAddrs);
	#endif

		if ( cpciIpmbAddrs > 0 )
		{
			cpciSlotId = ((cpciIpmbAddrs - 0xB0)/2)+1;

	#ifdef DEBUG
			sysDebugPrintf("%s: amcSlotId: 0x%02x\n", __FUNCTION__, cpciSlotId);
	#endif

			*((UINT16*)ptr) = (UINT16) cpciSlotId;
