...
/* Version control:
 *
 * $Header: /home/cvs/cvsroot/CCT_BIT_2/board/vp91x.01x/DispConfig.c,v 1.2 2015-02-27 08:52:53 mgostling Exp $
 *
 * $Log: DispConfig.c,v $
 * Revision 1.2  2015-02-27 08:52:53  mgostling
 * Replaced hard coded register numbers with literals
 *
...
TEST_INTERFACE (DispConfig, "Display Board Configuration")
{
	...
	reg32 = dIoReadReg(gpioBase+0x38,REG_32);//GP_LVL[63:32]
	reg = (reg32>>16)&1;
	sprintf (achBuffer, "Second Slot Board Interface: %s\n", ((reg & 0x1) == 0?"Available":"Not Available"));
	vConsoleWrite (achBuffer);
	...
	vConsoleWrite ("\nGPIO\n-------------------\n");	
	sprintf (achBuffer, "GPIO Direction      : GPIO0:%s\tGPIO1:%s\tGPIO2:%s \n",
			                                   ((reg & 0x10) == 0x10?"OUTPUT":"INPUT"),
											   ((reg & 0x20) == 0x20?"OUTPUT":"INPUT"),
											   ((reg & 0x40) == 0x40?"OUTPUT":"INPUT")
											   );
	vConsoleWrite (achBuffer);
	sprintf (achBuffer, "GPIO Current Data   : GPIO0:%x    \tGPIO1:%x    \tGPIO2:%x\n",
			                                   ((reg & 0x01) == 0x01?1:0),
											   ((reg & 0x02) == 0x02?1:0),
											   ((reg & 0x04) == 0x04?1:0)
											   );
...
void brdCuteInfo(brd_info *info)
{
	...
	pfa = PCI_MAKE_PFA (0, 0, 0);
//	info->memory = dPciReadReg (pfa, TOLUD, REG_16);
	info->memory = (UINT16)(dPciReadReg (pfa, TOLUD_IVYBRIDGE, REG_32) >> 16);		// Ivy Bridge TOLUD
